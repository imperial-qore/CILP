...
class CILP_TransProvisioner(Provisioner):
	...
	def provision(self):
		...
		opt = self.search(predips, self.env, self.maxv, self.window_buffer, self.host_util, self.model, self.optimizer, self.scheduler, self.loss_list, False)
		decision = opt.search()
		print(decision)
